...
class SearchController(QObject):
    ...
    def _initialize_pipeline(self) -> None:
        ...
        try:
            ...
            self.search_error.emit(f"Failed to initialize pipeline: {e!s}")
    ...
    def search(
        ...
        return self.task_runner.run_task(
            self._search_task, "search", query, max_results, document_id, page_number
        )
    ...
    def _search_task(
        ...
        try:
            ...
            raise Exception(f"Search failed: {e!s}") from e
    ...
    def get_similar_chunks(self, chunk_id: str, max_results: int = 5) -> str:
        ...
        return self.task_runner.run_task(
            self._similarity_task, "similarity", chunk_id, max_results
        )
    ...
    def _similarity_task(self, chunk_id: str, max_results: int) -> list[SearchResult]:
        ...
        try:
            ...
            # Find similar chunks using the vector database
            return vector_db.find_similar_chunks(chunk_id=chunk_id, limit=max_results)

            # Convert SearchResult objects to the expected format

        except Exception as e:
            raise Exception(f"Similarity search failed: {e!s}") from e
