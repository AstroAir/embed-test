...
class TaskRunner(QObject):
    ...
    def run_task(
        self, task_func: Callable, task_id: Optional[str] = None, *args, **kwargs
    ) -> str:
        """
        ...
        worker.signals.finished.connect(
            lambda result: self._on_task_finished(task_id, result)
        )
        worker.signals.error.connect(lambda error: self._on_task_error(task_id, error))
        worker.signals.progress.connect(
            lambda progress: self.task_progress.emit(task_id, progress)
        )
        worker.signals.status.connect(
            lambda status: self.task_status.emit(task_id, status)
        )
