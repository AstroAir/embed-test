...
def create_test_config(
    ...
    **overrides,
    ...
    pdf_config = PDFConfig(
        max_file_size_mb=10, timeout_seconds=60, extract_images=False
    )
    ...
    embedding_config = EmbeddingConfig(
        ...
        model_name=(
            "all-MiniLM-L6-v2"
            if embedding_type == EmbeddingModelType.SENTENCE_TRANSFORMERS
            else "text-embedding-3-small"
        ),
        batch_size=8,
        openai_api_key=(
            "test-key" if embedding_type == EmbeddingModelType.OPENAI else None
        ),
    )
...
def create_test_search_results(
    count: int = 3, document_id: str = "test_doc", base_score: float = 0.9
) -> list[SearchResult]:
...
def create_test_text_chunks(
    count: int = 3, base_text: Optional[str] = None
) -> list[TextChunk]:
...
def create_test_pdf_content(
    page_count: int = 2, realistic: bool = False
) -> dict[int, str]:
    ...
    def cleanup(self) -> None:
        ...
        for temp_file in self.temp_files:
            try:
                ...
                # Ignore cleanup errors but could be logged in production
