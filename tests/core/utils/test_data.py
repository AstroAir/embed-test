...
class DataGenerator:
    ...
    def generate_chunks(
        self, count: int = 5, min_words: int = 20, max_words: int = 80
    ) -> list[str]:
        """Generate a list of text chunks."""
        return [self.generate_text(min_words, max_words) for _ in range(count)]

    def generate_embeddings(
        self, count: int = 5, dimension: int = 384, normalize: bool = True
    ) -> list[list[float]]:
    ...
    def generate_metadata(
        self, document_id: Optional[str] = None, include_optional: bool = True
    ) -> dict[str, Any]:
    ...
    def generate_document_chunks(
        self, count: int = 5, document_id: str = "test_doc", embedding_dim: int = 384
    ) -> list[DocumentChunk]:
    ...
    def generate_search_results(
        self, count: int = 5, document_id: str = "test_doc"
    ) -> list[SearchResult]:
    ...
    def generate_text_chunks(
        self, count: int = 5, base_text: Optional[str] = None
    ) -> list[TextChunk]:
...
def create_sample_text(
    word_count: int = 50, include_special: bool = False, include_unicode: bool = False
) -> str:
    """Create sample text for testing."""
    generator = DataGenerator()
    return generator.generate_text(
        word_count, word_count, include_special, include_unicode
    )


def create_sample_chunks(
    count: int = 5, min_words: int = 20, max_words: int = 80
) -> list[str]:
