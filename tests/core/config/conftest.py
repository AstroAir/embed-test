...
def minimal_config() -> Config:
    ...
    return Config(debug=False, max_workers=1)
...
def config_with_all_sections(config_temp_dir: Path) -> Config:
    ...
    return Config(
        pdf=PDFConfig(max_file_size_mb=25, timeout_seconds=120, extract_images=False),
        ...
        logging=LoggingConfig(
            level=LogLevel.DEBUG, file_path=config_temp_dir / "test.log"
        ),
        debug=True,
        max_workers=3,
    )
