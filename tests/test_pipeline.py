...
from pdf_vector_system.pipeline import (
    PDFVectorPipeline, PipelineError, ProcessingResult
)
...
class TestPDFVectorPipeline:
    ...
    def test_initialization(self, test_config):
        ...
        with patch('pdf_vector_system.pipeline.PDFProcessor') as mock_pdf:
            with patch('pdf_vector_system.pipeline.TextProcessor') as mock_text:
                with patch('pdf_vector_system.pipeline.EmbeddingServiceFactory') as mock_factory:
                    with patch('pdf_vector_system.pipeline.ChromaDBClient') as mock_chroma:
    ...
    def test_process_pdf_success(self, test_config, temp_dir):
        ...
        mock_doc_chunks = [
            ...
            DocumentChunk(
                ...
                metadata={"document_id": "test_doc", "chunk_index": 1, "page_number": 2}
            )
        ]
        ...
        with patch('pdf_vector_system.pipeline.PDFProcessor') as mock_pdf_class:
            with patch('pdf_vector_system.pipeline.TextProcessor') as mock_text_class:
                with patch('pdf_vector_system.pipeline.EmbeddingServiceFactory') as mock_factory:
                    with patch('pdf_vector_system.pipeline.ChromaDBClient') as mock_chroma_class:
                        # Set up mocks
                        ...
                        mock_text_processor.clean_text.return_value = ("cleaned text", {})
                        mock_text_processor.chunk_text_with_metadata.return_value = mock_chunks
                        ...
                        with patch.object(pipeline, '_create_document_chunks') as mock_create_chunks:
                            ...
                            # Verify method calls
                            mock_pdf_processor.extract_text.assert_called_once_with(pdf_path)
            ...
            with patch('pdf_vector_system.pipeline.TextProcessor') as mock_text_class:
                with patch('pdf_vector_system.pipeline.EmbeddingServiceFactory') as mock_factory:
                    with patch('pdf_vector_system.pipeline.ChromaDBClient') as mock_chroma_class:
                        ...
                        mock_pdf_processor = Mock()
                        mock_pdf_processor.extract_text.side_effect = Exception("PDF extraction failed")
                        mock_pdf_class.return_value = mock_pdf_processor
        ...
        with patch('pdf_vector_system.pipeline.PDFProcessor'):
            with patch('pdf_vector_system.pipeline.TextProcessor'):
                with patch('pdf_vector_system.pipeline.EmbeddingServiceFactory'):
                    with patch('pdf_vector_system.pipeline.ChromaDBClient') as mock_chroma_class:
                        mock_vector_db = Mock()
                        mock_vector_db.search_documents.return_value = mock_search_results
                        mock_chroma_class.return_value = mock_vector_db
        ...
        with patch('pdf_vector_system.pipeline.PDFProcessor'):
            with patch('pdf_vector_system.pipeline.TextProcessor'):
                with patch('pdf_vector_system.pipeline.EmbeddingServiceFactory'):
                    with patch('pdf_vector_system.pipeline.ChromaDBClient') as mock_chroma_class:
                        mock_vector_db = Mock()
                        mock_vector_db.search_documents.side_effect = Exception("Search failed")
                        mock_chroma_class.return_value = mock_vector_db
                        
                        pipeline = PDFVectorPipeline(test_config)
                        
                        with pytest.raises(PipelineError, match="Error searching documents"):
        ...
        with patch('pdf_vector_system.pipeline.PDFProcessor'):
            with patch('pdf_vector_system.pipeline.TextProcessor'):
                with patch('pdf_vector_system.pipeline.EmbeddingServiceFactory'):
                    with patch('pdf_vector_system.pipeline.ChromaDBClient') as mock_chroma_class:
                        mock_vector_db = Mock()
                        mock_vector_db.delete_documents_by_filter.side_effect = Exception("Delete failed")
                        mock_chroma_class.return_value = mock_vector_db
                        
                        pipeline = PDFVectorPipeline(test_config)
                        
                        with pytest.raises(PipelineError, match="Error deleting document"):
            ...
            with patch('pdf_vector_system.pipeline.TextProcessor'):
                with patch('pdf_vector_system.pipeline.EmbeddingServiceFactory'):
                    with patch('pdf_vector_system.pipeline.ChromaDBClient') as mock_chroma_class:
                        ...
                        mock_vector_db.get_document_info.assert_called_once_with("doc_1")
    ...
    def test_create_document_chunks(self, test_config):
        ...
        mock_text_chunks = [
            Mock(content="Chunk 1", chunk_index=0, page_number=1, start_char=0, end_char=7),
            Mock(content="Chunk 2", chunk_index=1, page_number=1, start_char=8, end_char=15)
        ]
