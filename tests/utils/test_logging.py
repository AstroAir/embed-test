...
from pdf_vector_system.utils.logging import (
    LoggerMixin, setup_logging, get_logger, log_function_call,
    log_performance, log_error, log_warning, log_progress
)
...
class TestLoggerMixin:
    ...
    def test_logger_property(self):
        ...
        # The logger should be bound with the class name
...
class TestSetupLogging:
    ...
    def test_setup_logging_custom_format(self, mock_logger):
        ...
        assert any(custom_format in str(arg) for arg in call_args[0] + tuple(call_args[1].values()))
    ...
    def test_setup_logging_rotation_and_retention(self, mock_logger, temp_dir):
        ...
        assert "rotation" in call_kwargs or any("5 MB" in str(arg) for arg in mock_logger.add.call_args[0])
        assert "retention" in call_kwargs or any("14 days" in str(arg) for arg in mock_logger.add.call_args[0])
