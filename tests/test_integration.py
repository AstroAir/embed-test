...
from pdf_vector_system.config.settings import Config, PDFConfig, TextProcessingConfig, EmbeddingConfig, ChromaDBConfig
from pdf_vector_system.pipeline import PDFVectorPipeline
from pdf_vector_system.vector_db.models import SearchQuery, DocumentChunk
from tests.mocks.pdf_mocks import create_mock_pdf_document
from tests.mocks.embedding_mocks import MockEmbeddingService, MockBatchEmbeddingProcessor
...
class TestPDFToVectorIntegration:
    ...
    def test_pdf_to_chunks_integration(self, temp_dir):
        ...
        with patch('pdf_vector_system.pdf.processor.fitz.open') as mock_fitz:
            ...
            with patch('pdf_vector_system.embeddings.factory.SentenceTransformersService') as mock_st_service:
                with patch('pdf_vector_system.vector_db.chromadb_client.chromadb.PersistentClient') as mock_chroma_client:
                    # Set up embedding service mock
                    mock_embedding_service = MockEmbeddingService("test-model", embedding_dim=384)
        ...
        with patch('pdf_vector_system.pdf.processor.fitz.open') as mock_fitz:
            with patch('pdf_vector_system.embeddings.factory.SentenceTransformersService') as mock_st_service:
                with patch('pdf_vector_system.vector_db.chromadb_client.chromadb.PersistentClient') as mock_chroma_client:
                    ...
                    def mock_fitz_side_effect(path):
                        if "doc1.pdf" in str(path):
                            return create_mock_pdf_document(
                                page_count=1,
                                content_per_page=["Machine learning is a subset of artificial intelligence."]
                            )
                        else:
                            return create_mock_pdf_document(
                                page_count=1,
                                content_per_page=["Deep learning uses neural networks for pattern recognition."]
                            )
                    ...
                    assert any("machine learning" in result.content.lower() or 
                             "artificial intelligence" in result.content.lower() 
...
class TestConfigurationIntegration:
    ...
    def test_config_propagation(self, temp_dir):
        ...
        with patch('pdf_vector_system.pdf.processor.fitz.open'):
            with patch('pdf_vector_system.embeddings.factory.SentenceTransformersService') as mock_st_service:
                with patch('pdf_vector_system.vector_db.chromadb_client.chromadb.PersistentClient') as mock_chroma_client:
        ...
        with patch('pdf_vector_system.pdf.processor.fitz.open') as mock_fitz:
            with patch('pdf_vector_system.embeddings.factory.SentenceTransformersService') as mock_st_service:
                with patch('pdf_vector_system.vector_db.chromadb_client.chromadb.PersistentClient') as mock_chroma_client:
                    ...
                    mock_embedding_service.embed_texts.side_effect = Exception("Embedding service error")
                    mock_st_service.return_value = mock_embedding_service
...
class TestPerformanceIntegration:
    ...
    def test_batch_processing_performance(self, temp_dir):
        ...
        large_content = " ".join([f"This is sentence {i} in a large document." for i in range(50)])
        ...
        with patch('pdf_vector_system.pdf.processor.fitz.open') as mock_fitz:
            with patch('pdf_vector_system.embeddings.factory.SentenceTransformersService') as mock_st_service:
                with patch('pdf_vector_system.vector_db.chromadb_client.chromadb.PersistentClient') as mock_chroma_client:
                    ...
                    mock_doc = create_mock_pdf_document(
                        page_count=1,
                        content_per_page=[large_content]
                    )
                    mock_fitz.return_value = mock_doc
