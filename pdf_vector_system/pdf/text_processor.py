...
class TextProcessor(LoggerMixin):
    ...
    def validate_chunks(self, chunks: List[TextChunk]) -> Dict[str, Any]:
        ...
        if small_chunks:
            warnings.append(f"Found {len(small_chunks)} chunks smaller than {min_size} characters")
        ...
        if large_chunks:
            warnings.append(f"Found {len(large_chunks)} chunks larger than expected ({max_size} chars)")
    ...
    def remove_headers_footers(self, text: str, threshold: int = 3) -> str:
        ...
        if lines_to_remove:
            self.logger.debug(f"Removed {len(lines_to_remove)} repeated header/footer patterns")
