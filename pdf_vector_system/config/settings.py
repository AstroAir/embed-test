...
class PDFConfig(BaseModel):
    ...
    timeout_seconds: int = Field(default=300, description="PDF processing timeout in seconds")
    extract_images: bool = Field(default=False, description="Whether to extract images from PDFs")
    
    @validator('max_file_size_mb')
    def validate_file_size(cls: Type["PDFConfig"], v: int) -> int:
        if v <= 0:
            raise ValueError("max_file_size_mb must be positive")
        return v

    @validator('timeout_seconds')
    def validate_timeout(cls: Type["PDFConfig"], v: int) -> int:
        if v <= 0:
...
class TextProcessingConfig(BaseModel):
    ...
    def validate_chunk_size(cls: Type["TextProcessingConfig"], v: int) -> int:
        if v <= 0:
            raise ValueError("chunk_size must be positive")
        return v

    @validator('chunk_overlap')
    def validate_chunk_overlap(cls: Type["TextProcessingConfig"], v: int, values: Dict[str, Any]) -> int:
        if v < 0:
    ...
    def validate_min_chunk_size(cls: Type["TextProcessingConfig"], v: int) -> int:
        if v <= 0:
...
class EmbeddingConfig(BaseModel):
    ...
    timeout_seconds: int = Field(default=60, description="Timeout for embedding generation")
    ...
    cohere_base_url: Optional[str] = Field(default=None, description="Cohere API base URL")
    ...
    huggingface_cache_dir: Optional[str] = Field(default=None, description="Hugging Face cache directory")
    huggingface_device: Optional[str] = Field(default=None, description="Device for Hugging Face models (cpu, cuda, etc.)")
    huggingface_trust_remote_code: bool = Field(default=False, description="Trust remote code for Hugging Face models")

    # Google USE specific settings
    google_use_cache_dir: Optional[str] = Field(default=None, description="Google USE model cache directory")
    ...
    google_gemini_base_url: Optional[str] = Field(default=None, description="Google Gemini API base URL")
    google_gemini_project_id: Optional[str] = Field(default=None, description="Google Cloud project ID for Vertex AI")
    google_gemini_location: str = Field(default="us-central1", description="Google Cloud location for Vertex AI")

    # Advanced batch processing settings
    adaptive_batch_sizing: bool = Field(default=True, description="Enable adaptive batch sizing")
    memory_limit_mb: Optional[int] = Field(default=None, description="Memory limit for batch processing in MB")
    parallel_batches: Optional[int] = Field(default=None, description="Number of parallel batches")
    
    @validator('batch_size')
    def validate_batch_size(cls: Type["EmbeddingConfig"], v: int) -> int:
        if v <= 0:
            raise ValueError("batch_size must be positive")
        return v

    @validator('max_retries')
    def validate_max_retries(cls: Type["EmbeddingConfig"], v: int) -> int:
        if v < 0:
            raise ValueError("max_retries must be non-negative")
        return v

    @validator('memory_limit_mb')
    def validate_memory_limit(cls: Type["EmbeddingConfig"], v: Optional[int]) -> Optional[int]:
        if v is not None and v <= 0:
            raise ValueError("memory_limit_mb must be positive")
        return v

    @validator('parallel_batches')
    def validate_parallel_batches(cls: Type["EmbeddingConfig"], v: Optional[int]) -> Optional[int]:
        if v is not None and v <= 0:
            raise ValueError("parallel_batches must be positive")
        return v

    @validator('azure_openai_api_version')
    def validate_azure_api_version(cls: Type["EmbeddingConfig"], v: str) -> str:
    ...
    @validator('google_use_version')
    def validate_google_use_version(cls: Type["EmbeddingConfig"], v: str) -> str:
        valid_versions = ["4", "5"]
        if v not in valid_versions:
            raise ValueError(f"google_use_version must be one of {valid_versions}")
        return v

    @validator('google_gemini_location')
    def validate_google_gemini_location(cls: Type["EmbeddingConfig"], v: str) -> str:
        # Basic validation for Google Cloud location format
        if not v or len(v) < 3:
            raise ValueError("google_gemini_location must be a valid Google Cloud location")
        return v
...
class ChromaDBConfig(BaseModel):
    ...
    def validate_collection_name(cls: Type["ChromaDBConfig"], v: str) -> str:
        if not v or not v.strip():
            raise ValueError("collection_name cannot be empty")
        # ChromaDB collection names must be alphanumeric with underscores/hyphens
        if not v.replace('_', '').replace('-', '').isalnum():
            raise ValueError("collection_name must be alphanumeric with underscores or hyphens")
        return v

    @validator('distance_metric')
    def validate_distance_metric(cls: Type["ChromaDBConfig"], v: str) -> str:
    ...
    def validate_max_results(cls: Type["ChromaDBConfig"], v: int) -> int:
        if v <= 0:
...
class Config(BaseSettings):
    ...
    def validate_max_workers(cls: Type["Config"], v: int) -> int:
        if v <= 0:
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if self.embedding.model_type == EmbeddingModelType.OPENAI:
            ...
            if not self.embedding.azure_openai_endpoint:
                self.embedding.azure_openai_endpoint = os.getenv("AZURE_OPENAI_ENDPOINT")
