...
class OpenAIEmbeddingService(EmbeddingService):
    ...
    def embed_texts(self, texts: List[str]) -> EmbeddingResult:
        ...
        try:
            ...
            with PerformanceTimer(f"Generating OpenAI embeddings for {len(texts)} texts", log_result=False):
    ...
    def embed_single(self, text: str) -> List[float]:
        ...
        try:
            ...
            error_msg = f"Failed to generate OpenAI embedding for single text: {str(e)}"
            self.logger.error(error_msg)
            raise EmbeddingServiceError(error_msg) from e
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries:
                    ...
                    continue
                raise EmbeddingServiceError(f"Rate limit exceeded after {self.max_retries} retries") from e
                ...
                if attempt < self.max_retries and status_code and status_code >= 500:
                    ...
                    time.sleep(wait_time)
                    continue
                raise EmbeddingServiceError(f"OpenAI API error: {str(e)}") from e
                
            except Exception as e:
                if attempt < self.max_retries:
                    wait_time = 2 ** attempt
                    self.logger.warning(
                        f"Request failed, waiting {wait_time}s before retry {attempt + 1}/{self.max_retries}: {str(e)}"
                    )
                    time.sleep(wait_time)
                    continue
                raise EmbeddingServiceError(f"Request failed after {self.max_retries} retries: {str(e)}") from e
    ...
    def estimate_cost(self, texts: List[str]) -> Dict[str, Any]:
        ...
        estimated_tokens = sum(len(text.split()) * 1.3 for text in texts)  # Rough approximation
        ...
        price_per_token = pricing.get(self.model_name, 0.0001 / 1000)  # Default fallback
