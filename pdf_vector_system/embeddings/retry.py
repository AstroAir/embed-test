...
class RetryHandler(LoggerMixin):
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if attempt < self.config.max_retries:
                    ...
                    self.logger.warning(
                        f"{self.name} attempt {attempt + 1} failed ({failure_type}): {str(e)}. "
    ...
    def _is_circuit_breaker_open(self) -> bool:
        ...
        if stats.state == CircuitBreakerState.CLOSED:
            ...
            if (stats.last_failure_time and 
                time.time() - stats.last_failure_time >= self.config.circuit_breaker_recovery_timeout):
    ...
    def _record_failure(self) -> None:
        ...
        if (stats.state == CircuitBreakerState.CLOSED and 
            stats.failure_count >= self.config.circuit_breaker_failure_threshold):
    ...
    def _classify_failure(self, exception: Exception) -> FailureType:
        ...
        elif "timeout" in exception_str or "timeout" in exception_type:
            return FailureType.TIMEOUT
        elif "network" in exception_str or "connection" in exception_str:
            return FailureType.NETWORK_ERROR
        elif "401" in exception_str or "403" in exception_str or "auth" in exception_str:
...
def with_retry(config: RetryConfig, name: Optional[str] = None) -> Callable[[Callable[..., T]], Callable[..., T]]:
    """
...
class RetryMonitor(LoggerMixin):
    ...
    def get_global_stats(self) -> Dict[str, Any]:
        ...
        if stats["total_requests"] > 0:
            stats["global_failure_rate"] = float(stats["total_failures"]) / float(stats["total_requests"])
        else:
...
class ProviderCircuitBreaker(LoggerMixin):
    ...
    def can_execute(self) -> bool:
        ...
        if self.state == CircuitBreakerState.CLOSED:
            ...
            if (self.last_failure_time and
                ...
                self.success_count = 0
                self.logger.info(f"Circuit breaker for {self.provider_name} moved to HALF_OPEN")
                return True
            return False
        else:  # HALF_OPEN
            return True
    ...
    def record_success(self) -> None:
        ...
        if self.state == CircuitBreakerState.HALF_OPEN:
            ...
            if self.success_count >= self.success_threshold:
                ...
                self.failure_count = 0
                self.success_count = 0
                self.logger.info(f"Circuit breaker for {self.provider_name} moved to CLOSED")
    ...
    def force_open(self) -> None:
        ...
        self.state = CircuitBreakerState.OPEN
        self.last_failure_time = time.time()
        self.logger.warning(f"Circuit breaker for {self.provider_name} manually forced to OPEN")
    ...
    def force_close(self) -> None:
        ...
        self.failure_count = 0
        self.success_count = 0
        self.logger.info(f"Circuit breaker for {self.provider_name} manually forced to CLOSED")
