...
from pdf_vector_system.embeddings.base import EmbeddingService
from pdf_vector_system.embeddings.provider_configs import ProviderHealthCheckConfigs
...
class ProviderHealthChecker(LoggerMixin):
    ...
    def __init__(
        ...
        self.config = config or ProviderHealthCheckConfigs.get_health_check_config(
            provider_name
        )
    ...
    def check_health(self, force: bool = False) -> HealthCheckResult:
        ...
        try:
            ...
            self.logger.warning(
                ...
                f"(failures: {self.consecutive_failures}/{self.config['failure_threshold']}): {e!s}"
            )
    ...
    def get_health_summary(self) -> dict[str, Any]:
        ...
        recent_checks = [
            check
            for check in self.check_history[-20:]  # Last 20 checks
...
class HealthCheckManager(LoggerMixin):
    ...
    def check_provider_health(
        self, provider_name: str, force: bool = False
    ) -> HealthCheckResult:
        """
    ...
    def check_all_providers(self, force: bool = False) -> dict[str, HealthCheckResult]:
        ...
        for provider_name, checker in self.checkers.items():
            try:
                ...
                self.logger.error(f"Health check failed for {provider_name}: {e!s}")
                results[provider_name] = HealthCheckResult(
                    provider_name=provider_name,
                    status=HealthStatus.UNHEALTHY,
                    response_time=0.0,
                    error_message=f"Health check exception: {e!s}",
                )
