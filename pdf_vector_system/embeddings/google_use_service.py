...
class GoogleUSEService(EmbeddingService):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self.retry_handler = RetryHandler(retry_config, f"GoogleUSEService-{model_name}")
        ...
        try:
            ...
            with PerformanceTimer(f"Loading Google USE model {self.model_name}"):
                ...
                if not model_config:
                    ...
                    model_url = f"https://tfhub.dev/google/{self.model_name}/{self.version}"
                else:
            ...
            error_msg = f"Failed to load Google USE model {self.model_name}: {str(e)}"
            self.logger.error(error_msg)
            raise ModelNotFoundError(error_msg) from e
    ...
    def embed_texts(self, texts: List[str]) -> EmbeddingResult:
        ...
        try:
            ...
            with PerformanceTimer(f"Generating Google USE embeddings for {len(texts)} texts", log_result=False):
            ...
            error_msg = f"Failed to generate Google USE embeddings: {str(e)}"
            self.logger.error(error_msg)
    ...
    def embed_single(self, text: str) -> List[float]:
        ...
        try:
            ...
            error_msg = f"Failed to generate Google USE embedding for single text: {str(e)}"
            self.logger.error(error_msg)
            raise EmbeddingServiceError(error_msg) from e
    ...
    def health_check(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Google USE health check failed: {str(e)}")
            return False
