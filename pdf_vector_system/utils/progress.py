...
class ProgressTracker:
    ...
    def add_task(
        self, name: str, description: str, total: Optional[int] = None, **kwargs: Any
    ) -> str:
        """
...
class PerformanceTimer:
    ...
    def __init__(self, operation_name: str = "operation", log_result: bool = True):
        """
    ...
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
        ...
        if self.log_result and self.duration is not None:
            logger.info(
                f"Performance: {self.operation_name} completed in {self.duration:.2f}s"
            )
    ...
    def stop(self) -> float:
        ...
        if self.log_result:
            logger.debug(f"Stopped timing: {self.operation_name} - {duration:.2f}s")
    ...
    def lap(self, lap_name: str = "lap") -> float:
        ...
        # Store lap
        self.laps.append((lap_name, lap_duration))
