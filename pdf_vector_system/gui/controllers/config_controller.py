...
class ConfigController(QObject):
    ...
    def update_field(self, field_path: str, value: Any) -> bool:
        ...
        try:
            ...
            is_valid, error_msg = ConfigValidator.validate_field(field_path, value, self.config)
            ...
            if not is_valid:
                self.validation_error.emit(f"Configuration validation failed: {'; '.join(errors)}")
                return False
            ...
            self.operation_error.emit(f"Failed to update {field_path}: {str(e)}")
            return False
    ...
    def apply_config(self, config_dict: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.operation_error.emit(f"Failed to apply configuration: {str(e)}")
            return False
    ...
    def reset_to_defaults(self) -> bool:
        ...
        try:
            ...
            self.operation_error.emit(f"Failed to reset configuration: {str(e)}")
            return False
    ...
    def save_to_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            self.operation_error.emit(f"Failed to save configuration: {str(e)}")
            return False
    ...
    def load_from_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            def convert_strings_to_paths(obj, key=None):
                ...
                elif isinstance(obj, str) and key and ('directory' in key.lower() or 'path' in key.lower()):
                    return Path(obj)
                else:
                    return obj
            ...
            if not is_valid:
                self.validation_error.emit(f"Loaded configuration is invalid: {'; '.join(errors)}")
                return False
            ...
            self.operation_error.emit(f"Failed to load configuration: {str(e)}")
            return False
