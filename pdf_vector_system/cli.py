    ...
    try:
        ...
        for i, pdf_file in enumerate(valid_files, 1):
            ...
            try:
                ...
                if result.success:
                    console.print(f"✅ Success: {result.chunks_processed} chunks in {result.processing_time:.2f}s", style="green")
                else:
        ...
        console.print(f"❌ Pipeline error: {str(e)}", style="red")
        if debug:
...
def search(
    ...
    document_id: Optional[str] = typer.Option(None, "--document", "-d", help="Search within specific document"),
    page_number: Optional[int] = typer.Option(None, "--page", "-p", help="Search within specific page"),
    collection_name: Optional[str] = typer.Option(None, "--collection", "-c", help="Collection name"),
    score_threshold: float = typer.Option(0.0, "--threshold", "-t", help="Minimum similarity score"),
    ...
    try:
        ...
        console.print(f"❌ Search error: {str(e)}", style="red")
        if debug:
            console.print_exception()
        raise typer.Exit(1)
...
def stats(
    ...
    try:
        ...
        console.print(f"📊 Collection Statistics")
        console.print(f"📦 Collection: {pipeline.config.chroma_db.collection_name}")
        ...
        console.print(f"❌ Stats error: {str(e)}", style="red")
        if debug:
            console.print_exception()
        raise typer.Exit(1)
...
def list_docs(
    ...
    try:
        ...
        console.print(f"📋 Documents in Collection")
        console.print(f"📦 Collection: {pipeline.config.chroma_db.collection_name}")
        ...
        table = Table(title=f"Documents (showing up to {limit})", show_header=True, header_style="bold magenta")
        ...
        console.print(f"❌ List error: {str(e)}", style="red")
        if debug:
            console.print_exception()
        raise typer.Exit(1)
...
def delete(
    ...
    try:
        ...
        # Confirm deletion
        if not confirm:
            confirmed = typer.confirm(f"Delete document '{document_id}' from collection '{pipeline.config.chroma_db.collection_name}'?")
            if not confirmed:
        ...
        console.print(f"✅ Deleted document '{document_id}' ({chunks_deleted} chunks)", style="green")
...
def health(
    ...
    try:
        ...
        console.print(f"❌ Health check error: {str(e)}", style="red")
        if debug:
            console.print_exception()
        raise typer.Exit(1)
